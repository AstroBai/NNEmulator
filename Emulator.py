...
class ANN_Emu(nn.Module):
    ...
    def __init__(self, config_path='config.yaml'):
        ...
        Train_X = torch.tensor(np.load(data_config['Training_ParamPath']), dtype=torch.float32)
        self.Train_X_original = Train_X.clone()
        self.Train_Y_original = torch.tensor(np.load(data_config['Training_DataPath']), dtype=torch.float32)
        Trial_X = torch.tensor(np.load(data_config['Trial_ParamPath']), dtype=torch.float32)
        ...
        xs = np.load(data_config['xs_Path'])  # assuming x's are the same for train and trial
        if data_config['Do_Log']:
            Train_Y = torch.log(self.Train_Y_original + 1e-10)  # add small value to avoid log(0)
            Trial_Y = torch.log(self.Trial_Y_original + 1e-10)
        else:
            Train_Y = self.Train_Y_original
            Trial_Y = self.Trial_Y_original
        ...
        if ann_param.get('Positive_Output', False):
    ...
    def predict(self, x, xs=None):
        ...
        # load model
        model_path = os.path.join(self.config['OutputParam']['Model_SavePath'], 'ann_emu.pth')
        ...
        with torch.no_grad():
            ...
            if xs is not None:
                # interpolate with cubic spline to original xs
                xs_original = self.xs
                pred_interp = CubicSpline(xs_original, predictions.T)
                predictions = pred_interp(xs).T
...
if __name__ == "__main__":
    ...
    plt.figure(figsize=(10, 8))
    for i in range(len(trial_X_np)):
        color = plt.cm.viridis(i / len(trial_X_np))
        plt.plot(emu.xs, predictions[i], label=f'Trial {i+1} Prediction', color=color)
        plt.plot(emu.xs, trial_Y_np[i], '--', label=f'Trial {i+1} True', color=color)
    plt.legend()    
    plt.xlabel('X')
    plt.ylabel('Relative Error')
    plt.title('Emulator Predictions on Trial Set')
    plt.legend()
    plt.grid()
    plt.savefig(emu.config['OutputParam']['Model_SavePath'] + '/trial_predictions.png')